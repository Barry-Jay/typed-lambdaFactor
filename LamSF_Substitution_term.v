...
Lemma subst_rec_lift_rec3 : 
forall sch n k, subst_rec(lift_rec sch (S (n+k)) (S n)) (Ref n) (n+k) = lift_rec sch (n+k) n.
Proof.
induction sch; split_all. 
(* 2 *) 
unfold relocate. elim(test (S (n0+k)) n); elim(test (n0+k) n); split_all; try noway; insert_Ref_out; split_all.  
assert(n0 +k = n) by omega; subst. relocate_lt. 
replace (n0 + k + n0) with (n0 + (n0 + k)) by omega; auto. 
(* 1 *) 
replace(S(n+k)) with (n + S k) by omega. 
rewrite IHsch. auto. 
Qed. 

Ltac subst_lift_tac := repeat (rewrite subst_rec_lift_rec; try omega); repeat (rewrite lift_rec_null). 
