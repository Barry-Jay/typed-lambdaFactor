...
Ltac closed_tac M :=  repeat (rewrite (subst_rec_closed M); [| split_all; omega]). 
...
Lemma left_component_preserves_maxvar : forall M, 
...
Lemma right_component_preserves_maxvar : forall M, 
...
intros M N R; induction R; split_all; eauto; 
try (repeat (eapply2 max_monotonic); rewrite max_zero; omega); try omega; repeat (eapply2 max_max2); 
try (max_r; rewrite max_zero; omega); 
try (assert(max (max (maxvar M) (maxvar N)) (maxvar P) >= max (maxvar M) (maxvar N)) by max_l); 
try (repeat max_l; rewrite max_zero; omega); 
try (max_r; eapply2 left_component_preserves_maxvar; rewrite max_zero; omega);
try (max_r; eapply2 right_component_preserves_maxvar; rewrite max_zero; omega);
try (rewrite max_zero; assert(max(maxvar M) (maxvar N) >= maxvar M) by max_l; omega);
try (assert(max(maxvar M) (maxvar N) >= maxvar N) by max_r; omega);
try (max_l; max_l; eapply2 left_component_preserves_maxvar; rewrite max_zero; omega);
try (max_l; max_l; eapply2 right_component_preserves_maxvar; rewrite max_zero; omega);
try (max_l; max_r).
(* 2 *) 
unfold subst. 
...
Lemma status_lt_maxvar: forall M i, (status M = Active i \/ status M = Lam i) -> i < maxvar M.  
Proof. 
rank_tac. 
induction M; intros; try max_out; try (eapply2 IHM1); try (split_all; omega). 
(* 4 *) 
simpl in *; split_all.  inversion H0. 
(* 5 *) 
invsub; omega. 
(* 4 *) 
discriminate. 
(* 3 *) 
gen_case H0 o; simpl in *; or_tac. 
(* 2 *) 
simpl in *; inversion H0.
(* 3 *)  
gen2_case IHM H0 (status M); try ( inversion H0; discriminate). 
(* 4 *) 
gen2_case IHM H0 n; inversion H0; try discriminate.  inversion H2; subst. 
gen_case H1 (status M); gen_case H1 n0; discriminate. 
(* 3 *) 
gen2_case IHM H0 n; inversion H0; try discriminate.  inversion H2; subst. 
gen_case H1 (status M); gen_case H1 n0; discriminate. 
(* 2 *) 
gen3_case IHM H0 H1 (status M). 
(* 3 *) 
gen3_case IHM H0 H1 n. 
invsub. assert(S i < maxvar M).  eapply2 IHM; omega. omega. 
(* 2 *) 
gen3_case IHM H0 H1 n. 
invsub. assert(S i < maxvar M).  eapply2 IHM; omega. omega. 
(* 1 *) 
inversion H0. 
2: elim(lam_is_abs (App M1 M2) i); split_all; discriminate. 
assert(status M1 = Active i \/ status M2 = Active i \/ status M2 = Lam i) 
by eapply2 status_app_active. 
simpl in *.
gen4_case IHM1 H0 H1 H2 (status M1). 
invsub.
assert(max (maxvar M1 ) (maxvar M2) >= maxvar M1) by eapply2 max_is_max. 
assert(i<maxvar M1) by (eapply2 IHM1; omega). 
omega. 
(* 2 *) 
inversion H2. discriminate. 
(* 2 *) 
inversion H3; rewrite H4 in *. 
assert(max (maxvar M1 ) (maxvar M2) >= maxvar M2) by eapply2 max_is_max. 
assert(i<maxvar M2) by (eapply2 IHM2; omega). 
omega. 
(* 2 *) 
assert(max (maxvar M1 ) (maxvar M2) >= maxvar M2) by eapply2 max_is_max. 
assert(i<maxvar M2) by (eapply2 IHM2; omega). 
...
subst. inversion H3. simpl in *. assert False by eapply2 H7; noway. 
(* 2 *) 
gen4_case H1 H2 H3 H4 N; try discriminate.  
(* 3 *) 
assert(M=l). eapply2 star_monotonic. congruence. 
(* 2 *) 
subst. unfold abs_left in H3.  inversion H3. simpl in *. subst. 
assert False by eapply2 H7; noway. 
(* 1 *)
subst. gen2_case H4 H0 N; inversion H0; try discriminate. 
subst. inversion H0. simpl in *. assert False by eapply2 H7; noway. 
simpl in *. assert False by eapply2 H7; noway. 
Qed. 
...
Proof. 
unfold program, factorable; split_all. 
assert(status M <> Reducible). intro. 
assert(exists N, lamSF_red1 M N) by eapply2 Reducible_implies_reducible. 
split_all. eapply2 normal_is_irreducible.
assert(forall i, (status M <> Active i /\ status M <> Lam i)).
split_all; intro; assert(i< maxvar M) by eapply2 status_lt_maxvar; omega. 
gen2_case H H2 (status M); or_tac; auto 10.  
assert False by eapply2 H2; noway.
assert(Active n <> Active n) by eapply2 H2. assert False by eapply2 H3; noway. 
Qed. 



Ltac factorable_tac M := assert(factorable M)  by (eapply2 programs_are_factorable; split_all; omega);
 match goal with | H: factorable M |- _ => inversion H; split_all end. 
